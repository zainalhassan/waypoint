...
export function TemplateSyncWizard({ preview }: TemplateSyncWizardProps) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      {step === 1 && (
        <SectionCard
          title={`Review changes from ${preview.sourceName}`}
          description={
            preview.pendingSourceSync
              ? "Compare what will change — update pending from the author."
          ...
          {preview.updated.length > 0 && (
            <section>
              <h3 className="mb-2 text-sm font-medium">
                Updated stages ({preview.updated.length})
              </h3>
              <ul className="space-y-1 text-sm text-muted-foreground">
      ...
      {step === 2 && preview.requiresWizard && (
        <SectionCard
          title="Move items from removed stages"
          description="The author removed stages that still have your items. Choose where each group should go."
          headerColor="var(--color-route-yellow)"
        >
          {preview.removedWithItems.map((impact) => (
            <div key={impact.slug} className="space-y-2 rounded-[var(--radius-button)] border border-border p-4">
              <div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  required
                  className="flex h-9 w-full rounded-[var(--radius-button)] border border-input bg-transparent px-3 py-1 text-sm"
                >
      ...
      {((step === 2 && !preview.requiresWizard) || (step === 3 && preview.requiresWizard)) && (
        <SectionCard
          title="Confirm sync"
          description="Your template will match the source. Pipelines created from this template will be updated."
